...
class CompactUser {
    ...
    this.tscore_per_play = (this.total_score ?? 0) / (this.play_count ?? 1)
    this.rscore_per_play = (this.ranked_score ?? 0) / (this.play_count ?? 1)
    this.hits_per_play = (this.total_hits ?? 0) / (this.play_count ?? 1)
    }
}
